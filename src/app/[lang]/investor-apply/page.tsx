...
export default function InvestorApplyPage({ params: { lang } }: { params: { lang: string } }) {
  ...
  if (submitted) {
    return (
      ...
      >
        <div className="w-16 h-16 mx-auto mb-6 rounded-2xl bg-emerald/10 border border-emerald/20 flex items-center justify-center">
          <svg className="w-8 h-8 text-emerald" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        </div>
        <h2 className="text-xl font-bold text-white mb-2">Application Submitted</h2>
        <p className="text-sm text-slate-muted mb-8">Your investor application is under review. We&apos;ll notify you once approved.</p>
        <button
          onClick={() => router.push(`/${lang}/feed`)}
          className="px-6 py-2.5 rounded-xl bg-violet text-white text-sm font-semibold hover:brightness-110 transition-all"
        >
          Back to Feed
        </button>
      </motion.div>
      ...
      >
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-xs text-slate-muted">Submit your credentials for investor vetting</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-xs font-semibold text-slate-muted uppercase tracking-wider mb-1.5">LinkedIn Profile *</label>
            <input
              type="url"
              value={linkedinUrl}
              onChange={(e) => setLinkedinUrl(e.target.value)}
              className="w-full px-4 py-2.5 rounded-xl bg-onyx-800 border border-onyx-700/60 text-white text-sm placeholder:text-slate-muted/40 focus:outline-none focus:ring-2 focus:ring-violet/40 transition-all"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-muted uppercase tracking-wider mb-1.5">Company / Organization</label>
            <input
              type="text"
              value={companyName}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-muted uppercase tracking-wider mb-1.5">Professional Credential *</label>
            <div className="border-2 border-dashed border-onyx-700/60 rounded-xl p-4 text-center">
              ...
              <label htmlFor="credential-upload" className="cursor-pointer">
                {credentialFile ? (
                  ...
                  <div>
                    <svg className="w-6 h-6 text-slate-muted mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                    </svg>
                    <p className="text-xs text-slate-muted">Upload business card, certificate, or license</p>
                  </div>
                )}
              </label>
            </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-muted uppercase tracking-wider mb-1.5">Company Proof (optional)</label>
            <div className="border-2 border-dashed border-onyx-700/60 rounded-xl p-4 text-center">
              ...
              <label htmlFor="company-proof-upload" className="cursor-pointer">
                {companyProofFile ? (
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-muted">Upload company registration or proof of funds</p>
                  </div>
                )}
              </label>
            </div>
