    ...
    >
      <div className="flex items-start gap-3">
        <div className="relative shrink-0 mt-0.5">
          <div className="w-9 h-9 rounded-full bg-violet/10 flex items-center justify-center text-caption font-bold text-violet-light">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-0.5">
            <span className="text-body font-semibold text-white truncate">{thread.participantName}</span>
            <span className="text-micro font-mono text-subdued shrink-0">{thread.timestamp}</span>
          </div>
          <p className="text-caption text-muted truncate mb-1">{thread.lastMessage}</p>
          <div className="flex items-center gap-2">
            <span className={`w-1.5 h-1.5 rounded-full ${config.dot}`} />
            <span className="text-micro text-subdued">{config.label}</span>
            {thread.unread > 0 && (
              <span className="ml-auto w-5 h-4 rounded-full bg-violet/20 text-violet-light text-micro font-semibold flex items-center justify-center">
                {thread.unread}
              </span>
            )}
          </div>
    ...
    <div className="h-full flex flex-col bg-onyx-950/50 border-r border-onyx-700/60">
      <div className="shrink-0 px-4 py-3 border-b border-onyx-700/60">
        <div className="flex items-center justify-between">
          <h2 className="text-micro text-muted">Conversations</h2>
          <span className="text-micro font-mono text-subdued">{threads.length}</span>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto scrollbar-hide divide-y divide-white/[0.03]">
        {sections.map((section) => {
          if (section.threads.length === 0) return null;
          return (
            <div key={section.key}>
              <div className="px-4 py-2">
                <span className="text-micro text-subdued">{section.label}</span>
              </div>
        ...
        {threads.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
            <svg className="w-8 h-8 text-subdued mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 8.511c.884.284 1.5 1.128 1.5 2.097v4.286c0 1.136-.847 2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354 0-2.694-.055-4.02-.163a2.115 2.115 0 01-.825-.242m9.345-8.334a2.126 2.126 0 00-.476-.095 48.64 48.64 0 00-8.048 0c-1.131.094-1.976 1.057-1.976 2.192v4.286c0 .837.46 1.58 1.155 1.951m9.345-8.334V6.637c0-1.621-1.152-3.026-2.76-3.235A48.455 48.455 0 0011.25 3c-2.115 0-4.198.137-6.24.402-1.608.209-2.76 1.614-2.76 3.235v6.226c0 1.621 1.152 3.026 2.76 3.235.577.075 1.157.14 1.74.194V21l4.155-4.155" />
            </svg>
            <p className="text-caption text-muted">No conversations yet</p>
            <p className="text-micro text-subdued mt-1">Connect with founders to start chatting</p>
          </div>
        )}
      </div>
    </div>
